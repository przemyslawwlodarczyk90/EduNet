...
import { fetchContainerQuiz, submitContainerQuiz } from "../api";
...
export function QuizFromCatalog({ quizId }: QuizFromCatalogProps) {
  ...
  useEffect(() => {
    fetchContainerQuiz(quizId).then(setQuiz);
  }, [quizId]);
  ...
  const selectAnswer = async (optionIndex: number) => {
    ...
    if (questionIndex + 1 < quiz.questions.length) {
      ...
      const submitted = await submitContainerQuiz(quiz.id, nextAnswers);
      setResult(submitted);
    }
  };
